...
void runTestOnString(char* string, InstructionBuffer* instructionBuffer){
	...
	//while(attemptActiveOpt(instructionBuffer));
	
	//sanityCheck(instructionBuffer);
	
	//runGeneralExpressionOpt(instructionBuffer);
...
int main(){
	...
	//initInstructionBuffer(&instructionBuffer);
	/*
	addVariableToBlockFrame("var0 int",false,false,NULL);
	addVariableToBlockFrame("var1 int",false,false,NULL);
	addVariableToBlockFrame("var2 int",false,false,NULL);
	printf("Init done for test...\n\n");
	
	
	runTestOnString("var0=var1-var2;",&instructionBuffer);
	*/
	
	/*
	addVariableToBlockFrame("var0 long",false,false,NULL);
	//addVariableToBlockFrame("var1 long",false,false,NULL);
	runTestOnString("var0++;",&instructionBuffer);
	*/
	
	//addVariableToBlockFrame("var0 * int",false,false,NULL);
	//addVariableToBlockFrame("solutionCount int",false,false,NULL);
	addVariableToBlockFrame("i0 int",false,false,NULL);
	addVariableToBlockFrame("i1 int",false,false,NULL);

	//addGlobalFunction("fun0 ( i int ) void",false);

	
	initInstructionBuffer(&instructionBuffer);
	//runTestOnString("(var0[1])++;",&instructionBuffer);
	//runTestOnString("var0++;",&instructionBuffer);
	//runTestOnString("var2 = (var0 || var1);",&instructionBuffer);
	runTestOnString("i1=0xFFFF^(i0^0xFFFF);",&instructionBuffer);
	printf("\nFrom compiler:\n");
	printInstructionBuffer(&instructionBuffer);
	if (attemptPeepHoleOpt(&instructionBuffer)){
		printf("\nWith peephole:\n");
		printInstructionBuffer(&instructionBuffer);
	} else {
		printf("\nPeephole did nothing\n");
	}
	destroyInstructionBuffer(&instructionBuffer);
